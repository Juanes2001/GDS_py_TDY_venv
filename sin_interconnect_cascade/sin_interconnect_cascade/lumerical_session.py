...
API (`lumapi`) and a small helper to open an INTERCONNECT session.

This isolates the brittle, platform-specific path/DLL handling that the original
notebook repeated at the top of the Lumerical cell.  Importing `lumapi`
...
Typical use
-----------
    from .lumerical_session import open_interconnect
    ic = open_interconnect(hide_gui=True)
    try:
        ...                       # addelement / setnamed / run / getresult ...
    finally:
        ic.close()
"""
...
def open_interconnect(hide_gui: bool = False, version: str = LUMERICAL_VERSION):
    """
    Open and return a Lumerical INTERCONNECT session.
    ...
    -------
    lumapi.INTERCONNECT
        An open INTERCONNECT session.  The caller is responsible for `.close()`,
        ideally in a `finally:` block.
    """
    lumapi = import_lumapi(version)
    log.info("Launching Lumerical INTERCONNECT …")
    return lumapi.INTERCONNECT(hide=hide_gui)
